...
class BasemapSwitcher extends Component {
  ...
  componentDidMount() {
    this.setState({
      showBaseMapSwitcher: window.mapControls.basemap,
      toggleService: BasemapConfig.topoServices[1],
    });
    ...
    if (basemap === "TOPO") {
      ...
      for (let index = 0; index < this.state.topoLayers.length; index++) {
        ...
        if (layerName.toUpperCase() === name) {
          this.setState({
            topoActiveIndex: index,
            previousIndex: this.state.topoActiveIndex,
          });
      ...
      <div className={!this.state.showBaseMapSwitcher ? " sc-hidden" : ""}>
        <div id="sc-basemap-main-container">
          <div className={"sc-basemap-topo"}>
            <BasemapItem
              key={helpers.getUID()}
              className="sc-basemap-topo-toggle-item-container"
